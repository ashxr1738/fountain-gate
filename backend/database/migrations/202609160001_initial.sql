-- Church Equipment: intentionally small schema and three application roles.
...
begin
  ...
  values (
    new.id,
    coalesce(nullif(new.raw_user_meta_data->>'name', ''), nullif(new.raw_user_meta_data->>'full_name', ''), nullif(split_part(coalesce(new.email, ''), '@', 1), ''), 'Church member'),
...
create policy "users read own profile or admins read all" on public.users for select to authenticated using (id = auth.uid() or public.is_admin());
create policy "authenticated users read assets" on public.assets for select to authenticated using (true);
create policy "admins add assets" on public.assets for insert to authenticated with check (public.is_admin());
create policy "admins edit assets" on public.assets for update to authenticated using (public.is_admin()) with check (public.is_admin());
create policy "admins remove assets" on public.assets for delete to authenticated using (public.is_admin());
create policy "users read own requests; admins all" on public.requests for select to authenticated using (user_id = auth.uid() or public.is_admin());
create policy "users read own transactions; admins all" on public.transactions for select to authenticated using (user_id = auth.uid() or public.is_admin());

-- All status transitions happen here, not via a client-side update.
create or replace function public.request_asset(p_asset_id uuid, p_purpose text, p_requested_from timestamptz, p_requested_until timestamptz)
returns uuid language plpgsql security definer set search_path = public as $$
declare v_request_id uuid; v_status public.asset_status;
begin
  ...
  values(p_asset_id, auth.uid(), trim(p_purpose), p_requested_from, p_requested_until) returning id into v_request_id;
  update public.assets set status = 'REQUESTED' where id = p_asset_id;
  return v_request_id;
end; $$;

create or replace function public.decide_request(p_request_id uuid, p_approve boolean, p_reason text default null)
...
begin
  ...
  if p_approve then
    update public.requests set status = 'APPROVED', approved_at = now(), approved_by = auth.uid(), rejection_reason = null where id = p_request_id;
    update public.assets set status = 'CHECKED_OUT' where id = v_request.asset_id;
    insert into public.transactions(asset_id, user_id, request_id, action) values(v_request.asset_id, v_request.user_id, p_request_id, 'CHECKOUT');
  else
    update public.requests set status = 'REJECTED', rejection_reason = nullif(trim(coalesce(p_reason, '')), '') where id = p_request_id;
